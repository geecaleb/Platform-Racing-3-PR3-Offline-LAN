...
public class LevelData : IXmlSerializable
{
	...
	public LevelData(DbDataReader reader)
	{
		...
		this.AuthorUsername = reader["author_username"] as string ?? "Unknown";
		this.AuthorNameColor = Color.FromArgb((int)reader["author_name_color"]);

		this.Title = reader["title"] as string ?? "";
		this.Description = reader["description"] as string ?? "";
		this.Publish = (bool)reader["publish"];

		this.SongId = reader["song_id"] as string ?? "";
		...
		this.BgImage = reader["bg_image"] as string ?? "";
		this.Data = reader["level_data"] as string ?? "";
		this.Lua = reader["lua"] as string ?? "";
		...
		if (this.IsCampaign)
		{
			...
			this.CampaignSeason = reader["campaign_season"] as string ?? "";
		}
