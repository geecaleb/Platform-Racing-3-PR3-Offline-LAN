...
internal class ChatRoom
{
	...
	internal bool Join(ClientSession session, uint chatId = 0)
	{
		...
		// Show admin tip if user is admin and in chat-Home
		if (this.Name == "chat-Home" && session.HasPermission("command.list.use"))
		{
			session.SendPacket(new ChatOutgoingMessage("chat-Home", "# Type /list for a list of available admin commands", 0, 0, "", System.Drawing.Color.Black));
		}
