...
internal sealed class PlatformRacing3Server : PlatformRacing3.Server.API.Core.PlatformRacing3
{
	...
	private readonly UserVarsUpdateSubscriber userVarsUpdateSubscriber;

	private readonly CampaignManager campaignManager;

	private IListener listener;

	public PlatformRacing3Server(ILoggerFactory loggerFactory, IServiceProvider serviceProvider, ServerManager serverManager, ClientManager clientManager, UserVarsUpdateSubscriber userVarsUpdateSubscriber, CampaignManager campaignManager)
	{
		...
		this.userVarsUpdateSubscriber = userVarsUpdateSubscriber;
	...
	internal async Task Init()
	{
		...
		// Initialize the UserVarsUpdateSubscriber
		this.userVarsUpdateSubscriber.Initialize();
		...
		this.listener = IListener.CreateTcpListener(new IPEndPoint(IPAddress.Parse(PlatformRacing3Server.ServerConfig.BindIp), PlatformRacing3Server.ServerConfig.FlashPolicyPort), socket =>
		{
